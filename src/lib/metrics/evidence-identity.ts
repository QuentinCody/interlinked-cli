...
function supportHash(inventory: RepositoryInventory): string {
    const rows: string[][] = [];
    let bytes = 0;
...
export function evidenceIdentity(inventory: RepositoryInventory): EvidenceIdentity {
    return { sourceHash: inventory.sourceHash, inputHash: inventory.inputHash,
        ...
        scopeHash: hashBytes(JSON.stringify([inventory.version, inventory.excluded, inventory.gaps, inventory.issues])), supportHash: supportHash(inventory) };
}
