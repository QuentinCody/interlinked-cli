...
describe("coverage on both daemon transports", () => {
    it("routes control queries and leaves malformed event handling with the original loop", async () => {
        // SAFETY: the underlying event loop is mocked; this wrapper only reads ctx.
        const loop = createCoverageEventLoop({ ctx: {} } as EventLoopDeps);
        ...
        expect(JSON.parse(query.additional_context!)).toMatchObject({ readiness: "unmeasured" });
    ...
    it("preserves existing feedback and adds explicit unavailable coverage on raw and framed Stop", async () => {
        // SAFETY: the event loop is mocked; only this context field is read by the wrapper.
        const deps = { ctx: { hookCoverageUnavailable: "watcher offline" } } as EventLoopDeps;
