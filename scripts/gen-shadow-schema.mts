...
} from "../src/harness/shadow/protocol/registry.js";

const PROTOCOL_DIR = join(import.meta.dirname, "../src/harness/shadow/protocol");
...
function literalSchema(type: ts.Type): Schema | null {
	...
	if ((type.flags & ts.TypeFlags.BooleanLiteral) !== 0) {
		// SAFETY: a BooleanLiteral type is an IntrinsicType whose name is "true" or "false".
...
function elementTypeOf(type: ts.Type): ts.Type | null {
	...
	// SAFETY: an Array/ReadonlyArray type is always a TypeReference carrying its element argument.
...
function stable(value: unknown): unknown {
	...
	// SAFETY: null, arrays and non-objects were all returned above.
...
// `renderShadowSchema` from this module, and an import must never write the
// schemas it is checking.
