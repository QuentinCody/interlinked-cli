...
function spawnResult(over: {
	...
	// SAFETY: absent stdout/stderr is deliberate malformed process output that exercises the runner's text fallbacks.
	return { pid: 123, output: [], signal: null, stderr: "", ...over } as SpawnSyncReturns<string>;
}
...
describe("swift.ts mutation-kill w43", () => {
	...
	describe("runSwiftLint — only pushes --path when mode is exactly 'file'", () => {
		it("N: mode='project' with a targetFile set does NOT add --path", () => {
			spawnSyncMock.mockReturnValue(spawnResult({ error: undefined, stdout: "[]", status: 0 }));
